...
void movePos(float dx, float dy, Vector2 *pos, int screenWidth,
             int screenHeight) {
    float newX = pos->x + dx;
    float newY = pos->y + dy;
...
Color getRGBColor(float t) {
    return (Color){127.5f + 127.5f * sinf(t),
                   127.5f + 127.5f * sinf(t + 2.0944f),
...
void ball_demo(WindowOptions *options) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyDown(KEY_W))
            movePos(0, -speed * dt, &playerPos, screenWidth, screenHeight);
        if (IsKeyDown(KEY_A))
            movePos(-speed * dt, 0, &playerPos, screenWidth, screenHeight);
        if (IsKeyDown(KEY_S))
            movePos(0, speed * dt, &playerPos, screenWidth, screenHeight);
        if (IsKeyDown(KEY_D))
            movePos(speed * dt, 0, &playerPos, screenWidth, screenHeight);
